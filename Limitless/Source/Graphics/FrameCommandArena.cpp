...
namespace Limitless
{
    ...
    void FrameCommandArena::Initialize(size_t bytesPerPage, uint32_t bufferedFrames)
    {
        if (m_Initialized)
        {
            return;
        ...
        m_Initialized = true;
        LT_CORE_INFO("FrameCommandArena initialized (BytesPerPage={}, BufferedFrames={})", m_DefaultPageSize, m_SlotCount);
    }
    ...
    FrameCommandArena::Page& FrameCommandArena::GetOrAddPage(FrameSlot& slot, size_t minCapacity)
    {
    ...
    void* FrameCommandArena::Allocate(size_t sizeBytes, size_t alignment)
    {
        ...
        {
            LT_CORE_ERROR("FrameCommandArena: new page too small ({} < {})", newPage.CapacityBytes, newRequired);
            return nullptr;
        }
