...
namespace Limitless
{
    ...
    void FrameUploadAllocator::Initialize(size_t bytesPerFrame, uint32_t bufferedFrames)
    {
        if (m_Initialized)
        {
            return;
        ...
        m_Initialized = true;
        LT_CORE_INFO("FrameUploadAllocator initialized (BytesPerFrame={}, BufferedFrames={})", m_DefaultBytesPerFrame, m_BufferCount);
    }
    ...
    void FrameUploadAllocator::EnsureFrameCapacity(FrameBuffer& buffer, size_t requiredBytes)
    {
        ...
        // Grow to next power-of-two-ish size (avoid frequent reallocations).
