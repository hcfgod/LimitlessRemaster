...
class format_string_checker {
 ...
 public:
  ...
  FMT_CONSTEXPR auto on_format_specs(int id, const Char* begin, const Char* end)
    ...
    int bracket_count = 0;
    // walk until we hit the closing '}' that matches the opening one,
    // properly accounting for any nested replacement fields. Written as a
    // `while` loop to avoid a static-analysis false-positive about an
    // "ill-defined for-loop".
    while (begin != end && (*begin != '}' || bracket_count > 0)) {
      if (*begin == '{')
        ++bracket_count;
      else if (*begin == '}')
        --bracket_count;
